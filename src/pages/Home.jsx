...
import {
  ...
  toCurlyQuotes,
  toStraightQuotes,
} from "../lib/textCase.js";

const SAMPLE =
  "the quick brown fox jumps over the lazy dog: a study of speed and rest";
      ...
      <ul className="mt-3 space-y-1.5">
        {guide.rules.map((rule) => (
          <li key={rule} className="flex gap-2 text-sm leading-6 text-ink-700">
            <Icon.check
              className="mt-1.5 shrink-0 text-brand-600"
              width={14}
              height={14}
            />
            <span>{rule}</span>
    ...
    <section className="card p-5" aria-labelledby="score-heading">
      ...
      <ul className="mt-4 space-y-2">
        {analysis.checks.map((check) => (
          <li key={check.label} className="flex items-start gap-2 text-sm">
            <span
              className={`mt-0.5 shrink-0 ${check.ok ? "text-emerald-600" : "text-gray-300"}`}
            >
      ...
      <div className="mt-4 space-y-2 border-t border-gray-100 pt-4">
        ...
        <p className="text-sm text-ink-700">
          ...
          <span
            className={
              analysis.truncatedInGoogle ? "text-red-600" : "text-emerald-700"
            }
          >
            {analysis.truncatedInGoogle
              ? `Will be cut at about 60 characters (${analysis.characters})`
              : "Shows fully"}
          </span>
        </p>
        <p className="text-sm text-ink-700">
          ...
          <span
            className={
              analysis.truncatedInEmail ? "text-amber-600" : "text-emerald-700"
            }
          >
            {analysis.truncatedInEmail
              ? "Will probably be cut on phones (over 50)"
              : "Fits on phones"}
          </span>
        </p>
      </div>
...
function CustomDictionary({ dictionary, setDictionary }) {
  const update = (key) => (e) =>
    setDictionary({ ...dictionary, [key]: e.target.value });
  return (
    <section className="card p-5" aria-labelledby="dict-heading">
      ...
      <p className="mt-1 text-xs leading-5 text-ink-500">
        Words that must always look a certain way, whatever the style guide
        says. Separate them with commas. They are saved in this browser only.
      </p>
...
export default function Home() {
  ...
  const [preserveAcronyms, setPreserveAcronyms] = useLocalStorage(
    "cmt:preserveAcronyms",
    true,
  );
  const [preserveMixedCase, setPreserveMixedCase] = useLocalStorage(
    "cmt:preserveMixedCase",
    true,
  );
  ...
  const options = useMemo(
    () => ({
      preserveAcronyms,
      preserveMixedCase,
      alwaysUppercase: dictionary.alwaysUppercase
        .split(",")
        .map((w) => w.trim())
        .filter(Boolean),
      alwaysCapitalize: dictionary.alwaysCapitalize
        .split(",")
        .map((w) => w.trim())
        .filter(Boolean),
      alwaysLowercase: dictionary.alwaysLowercase
        .split(",")
        .map((w) => w.trim())
        .filter(Boolean),
    }),
  ...
  const styleTabs = STYLE_GUIDES.map((g) => ({
    key: g.key,
    label: g.label,
    title: g.name,
  }));
    ...
    <>
      ...
      <section className="border-b border-gray-200 bg-white">
        <div className="container-page py-8">
          ...
          <p className="mt-2.5 max-w-3xl text-base leading-7 text-ink-700">
            {HOME_PAGE.intro}
          </p>
      ...
      <div className="container-page py-8">
        ...
        <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-10">
          <div className="min-w-0">
                ...
                <div className="flex flex-wrap gap-2">
                  {CASE_ACTIONS.map((item) => (
                    <button
                      ...
                      className={
                        item.key === action ? "btn-primary" : "btn-secondary"
                      }
                    >
                      {item.label}
                    </button>
            ...
            <div className="mt-6">
              <StatGrid
                stats={[
                  ...
                  {
                    label: "Reading time",
                    value: `${Math.max(1, Math.round(stats.readingTimeSeconds / 60))} min`,
                  },
                ]}
              />
            </div>
            ...
            <section className="prose-basic mt-12 max-w-3xl">
              ...
              <p>
                Every title case rule comes down to one question: which words
                are important enough to get a capital letter? All eight title
                case guides on this page agree on the main part. The first word,
                the last word and every important word — nouns, pronouns, verbs,
                adjectives and adverbs — get a capital. They only disagree about
                the small words in between.
              </p>
              <p>
                <strong>APA, AP and AMA count letters.</strong> Any word of four
                letters or more gets a capital, whatever type of word it is. So
                you get <em>Between</em>, <em>Through</em> and <em>With</em>.
              </p>
              <p>
                <strong>Chicago and MLA look at the type of word.</strong> Every
                preposition stays small, however long it is. So the same title
                gives you <em>between</em>, <em>through</em> and <em>with</em>.
              </p>
              <p>
                <strong>Bluebook and Wikipedia are in the middle.</strong> They
                keep words of four letters or fewer small. The{" "}
                <strong>New York Times</strong> does not use a rule at all — it
                uses a fixed list of small words.
              </p>

              <h2>The rules everyone agrees on</h2>
              <ul>
                <li>
                  Always capitalize the first word, even if it is "a" or "the".
                </li>
                <li>
                  Always capitalize the first word after a colon. This is where
                  most mistakes happen.
                </li>
                <li>
                  Capitalize all nouns, verbs, adjectives and adverbs, however
                  short. "Is" and "Be" are verbs.
                </li>
                <li>
                  Never use capitals just to make a word look important. Use
                  italics instead.
                </li>
                <li>
                  Leave short forms as they are. NASA, HTML and PDF do not
                  become Nasa, Html and Pdf.
                </li>
              </ul>

              <h2>The other buttons, and when to use them</h2>
              <p>
                <strong>Sentence case</strong> capitalizes only the first word
                and names. Use it for headings inside a page, buttons, form
                labels and email subject lines.
              </p>
              <p>
                <strong>Proper case</strong> capitalizes the first letter of
                every word and makes the rest small. This is what you want for
                names and addresses exported from a database.
              </p>
              <p>
                <strong>First letter</strong> does the same but does not touch
                the other letters, so <em>iPhone</em> stays <em>iPhone</em>. And{" "}
                <strong>alternating</strong> and <strong>toggle</strong> case
                are mostly for jokes and memes, which is a perfectly good use of
                a free tool.
              </p>

              <h2>Fixing a full document</h2>
              <p>
                Paste as many lines as you want. Each line is handled on its
                own, so a list of forty headings comes back matching in one go.
                This is the fastest way to do the consistency check we explain
                in our self-editing checklist. It catches the mix of title case
                and sentence case that slowly creeps into any document written
                over a few weeks.
              </p>

              <h2>Which guide do Indian schools and colleges use?</h2>
              <p>
                Most Indian universities follow APA or MLA, and many management
                and commerce courses use APA. Science and medical journals
                usually ask for AMA or their own house style. Always check your
                department handbook first, because the guide named there is the
                one your marks depend on. If your workplace has no rule at all,
                use AP — it is the easiest to read and the hardest to get wrong.
              </p>
            </section>
        ...
        <section className="mt-16" aria-labelledby="all-tools-heading">
          <h2
            ...
            className="mb-1 text-2xl font-bold text-ink-900"
          >
            Everything else we make
          </h2>
          <p className="mb-6 text-sm text-ink-500">
            Free tools for writers, students, marketers and anyone who works
            with text every day.
          </p>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {TOOL_GROUPS.map((group) => (
              <div key={group.id}>
                <h3 className="mb-2 text-xs font-bold tracking-wide text-brand-600 uppercase">
                  {group.label}
                </h3>
